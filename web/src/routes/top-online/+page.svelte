<script lang="ts">
  ...
  const top = $derived(topByPlayers(games.data?.records ?? [], 100, (r) => r.type_game === "online"));
...
<svelte:head>
  <title>{t("nav.topOnline")} · Steam F2P Tracker</title>
  <meta name="description" content={t("topOnline.subtitle")} />
</svelte:head>

<PageHeader title={t("nav.topOnline")} subtitle={t("topOnline.subtitle")} />

<QueryState loading={games.loading && !games.data} error={games.error} retry={() => games.refetch()}>
  <div class="rounded-lg border bg-card p-4">
    <EChart {option} height={560} label={t("nav.topOnline")} />
  </div>
