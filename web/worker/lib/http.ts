 ...
 *  from web/static/_headers, so anything set there must be re-applied here. */

/**
 * Security headers applied to every Worker-generated response.
 *
 * Kept in step with web/static/_headers, which covers static assets only.
 ...
 * would govern nothing. The one kind of HTML the Worker does produce - the
 * admin SPA's shell, for each of shared/admin-routes.ts's ADMIN_ROUTES -
 * builds its own nonce CSP instead. What IS worth setting on a JSON body is a
 * frame/sniff/embed lockdown, which is what these do.
 */
export const SECURITY_HEADERS: Record<string, string> = {
  ...
  // Android builds run on tauri://localhost or http://tauri.localhost and fetch
  // /api/data/* and /img/* from this origin cross-site (API_ORIGIN in
  // src/lib/site.ts), so `same-site` would blank every image and stop the
  // catalogue loading in the packaged apps. web/static/_headers can and does
